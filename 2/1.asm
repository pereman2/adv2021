...
_start:
  ...
  mov rsi, world            ; address of string to output
...
set_u:  
  mov r10, depth                  ; let's use r10 to store if we change horizontal or depth 
  mov qword[sum], -1
  jmp wait_n
set_d:  
  mov r10, depth                  ; let's use r10 to store if we change horizontal or depth 
  ...
  jmp wait_n
set_f:  
  mov r10, hor                  ; let's use r10 to store if we change horizontal or depth 
  mov qword[sum], 1                   ; forward decreases
  jmp wait_n
wait_n: 
  call reset_buffer
  call read_chr
...
exit: 
  ;; end
  mov rsi, input              ; address of string to output
  mov rdx, 6                  ; number of bytes
  call print
